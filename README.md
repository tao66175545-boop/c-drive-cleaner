...
界面中的“智能助手”采用左右对话气泡：挥手的曙光云 IP 在左侧回复，用户在右侧发送消息，并使用内置的固定用户头像。它可以回答项目含义，也可以用自然语言控制工具内的受限操作，例如切换页面、开始或停止只读扫描、勾选低风险建议项、清空选择、查看报告和准备清理。所有动作都复用严格参数的 UI Broker；包含路径、PowerShell/命令行、绕过规则或替代用户确认的请求会被拒绝，最终删除仍必须由用户在原生确认框中批准。
...
旅行路由支持“飞猪规划北京周末一日游”“杭州自由行”“成都跟团游”和“川西自驾游”等表达。普通的周末话题和 C 盘扫描、清理命令不会进入飞猪流程。

FlyAI 返回的 Markdown 会在本机转换为结构化旅行结果，按摘要、交通、景点、住宿、预算和实时推荐分区展示，并把长 URL 收敛为可点击的“飞猪详情”。同一旅行问题可在隔离进程中追加一次受限关键词搜索，最多补充 3 张飞猪/阿里 CDN 官方预览图；图片受域名、HTTPS、数量、大小、超时、内容类型和解码尺寸限制，失败时自动降级为纯文字卡片。
